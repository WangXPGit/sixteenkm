/** 
 * ConnectionContext.java Create on 2017-12-21
 */
...
import com.sixteenkm.api.util.PropsUtil;
...
public class ConnectionContext {
	...
	static {
		...
		if (PropsUtil.containsKey(config, TEST_ON_BORROW)) {
			dataSource.setTestOnBorrow(PropsUtil.getBoolean(config,
					TEST_ON_BORROW));
		}
		if (PropsUtil.containsKey(config, TEST_ON_RETURN)) {
			dataSource.setTestOnReturn(PropsUtil.getBoolean(config,
					TEST_ON_RETURN));
		}
	}
