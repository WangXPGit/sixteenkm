/** 
 * DefaultDataAccessor.java Create on 2017年12月21日
 */

package com.sixteenkm.api.jdbc.dao.impl;
...
import com.sixteenkm.api.jdbc.paging.TotalCountHolder;
import com.sixteenkm.api.jdbc.dao.DataAccessor;
...
public class DefaultDataAccessor implements DataAccessor {
	...
	@Override
	public <K, V> Map<K, V> selectMap(String statementId, Object sqlParam,
			String mapKey) {
		checkStatementId(statementId);
		checkParameter(sqlParam);
		checkMapKey(mapKey);
	...
	@Override
	public <T> Paging<T> selectPaging(String statementId, int pageNumber,
			int pageSize) {
		checkStatementId(statementId);
	...
	@Override
	public <T> Paging<T> selectPaging(String statementId, Object sqlParam,
	...
	private <T> Paging<T> doSelectPaging(String statementId, Object sqlParam,
			int pageNumber, int pageSize) {
		RowBounds rowBounds = createRowBounds(pageNumber, pageSize);
		List<T> itemList = sqlSession.selectList(statementId, sqlParam,
				rowBounds);
		try {
	...
	private void checkPageNumber(int pageNumber) {
		if (pageNumber < 1) {
			throw new IllegalArgumentException(
					"argument [pageNumber] is invalid: " + pageNumber);
		}
	}
	...
	private void checkPageSize(int pageSize) {
		if (pageSize <= 0) {
			throw new IllegalArgumentException(
					"argument [pageSize] is invalid: " + pageSize);
		}
	}
